    ...
    <nav v-if="posts" aria-label="Page Navigation" class="g-mb-50">
      <ul class="list-inline">
        ...
        <li v-for="(page, index) in iter_pages" v-bind:key="index" class="list-inline-item g-hidden-sm-down">
          <router-link v-if="page != 'NaN'" v-bind:to="{ name: 'Home', query: { page: page, per_page: posts._meta.per_page }}" v-bind:class="{'u-pagination-v1-1--active': $route.query.page == page || (!$route.query.page && page == 1)}" class="u-pagination-v1__item u-pagination-v1-1 g-rounded-50 g-pa-12-19">{{ page }}</router-link>
          <span v-else class="g-pa-12-19">...</span>
        </li>
...
export default {
      ...
      this.$axios
        ...
        .then(response => {
          ...
          let arr = [
            1,
            2,
            this.posts._meta.page - 2,
            this.posts._meta.page - 1,
            this.posts._meta.page,
            this.posts._meta.page + 1,
            this.posts._meta.page + 2,
            this.posts._meta.total_pages - 1,
            this.posts._meta.total_pages
          ]
          // 小于1，或大于最大页数的都是非法的，要去除
          arr = arr.filter(
            item => item > 0 && item <= this.posts._meta.total_pages
          )
          // 去除重复项
  ...
  created() {
    ...
    if (this.$route.query.token) {
      window.localStorage.setItem('madblog-token', this.$route.query.token)
      store.loginAction()
    }
    this.getPosts()
